...
class ArtworkManager:
        ...
        try:
            ...
            if remove_source:
                try:
                    ...
                    logging.warning(
                        f"⚠️ Could not remove source file {source_path}: {e}"
                    )
    ...
    async def _copy_file(
        self, source_path: Path, target_path: Path, log_message: str = None
    ) -> bool:
        ...
        try:
            ...
            logging.error(
                f"💥 Error copying file from {source_path} to {target_path}: {e}"
            )
            return False
    ...
    async def resize_for_social(
        ...
        try:
            ...
            logging.debug(
                f"🖼️ Resized image for social media: {image_path.name} → {size[0]}x{size[1]}"
            )
    ...
    async def resize_and_convert_image(
        ...
        try:
            ...
            with Image.open(source_path) as img:
                ...
                new_img.save(target_path, format=format, quality=95)

                logging.debug(
                    f"🖼️ Processed image: {source_path.name} → {target_path.name} ({width}x{height})"
                )
                return True
